...
{
  char *fn_copy;
  char *f_name; // Stores actual progra name
  tid_t tid;
  ...
  f_name = strtok_r (f_name," ",&save_ptr);   /* Actual program name  */
...
process_wait (tid_t child_tid)
{
  ...
  // Finding the child process in current thread
  ...
  // If current thread's child process doesn't exist exit
  if(!ch || !e1)
    return -1;

  thread_current()->waitingon = ch->tid;

  // Check if child process is still active otherwise put it in wait
  if(!ch->used)
    sema_down(&thread_current()->child_lock);
...
process_exit (void)
{
  ...
  uint32_t *pd;
    ...
    /* Use lock to prevent interrupts */
...
setup_stack (void **esp, char * file_name)
{
  ...
  // Stores the address of arguments
  int *argv = calloc(argc,sizeof(int));

  // Iterate through arguments and place them in stack
  ...
  // Inserting zero in stack
  int zero = 0;
  *esp-=sizeof(int);
  memcpy(*esp,&zero,sizeof(int));

  // Putting addresses of arguments in user stack
  for(i=argc-1;i>=0;i--)
  {
    *esp-=sizeof(int);
  ...
  // Put the address of first argument put to stack
  int pt = *esp;
  *esp-=sizeof(int);
  memcpy(*esp,&pt,sizeof(int));

  // Put number of arguments in user stack
  *esp-=sizeof(int);
